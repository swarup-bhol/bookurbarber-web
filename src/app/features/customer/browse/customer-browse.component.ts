          ...
          <div style="display:flex;align-items:center;justify-content:space-between">
            <div>
              ...
              <div style="font-family:'Unbounded',sans-serif;font-size:20px;font-weight:700">
                Hi, {{ auth.user()?.fullName || 'there' }}!
              </div>
            </div>
            <button (click)="startEditName()"
              style="padding:8px 14px;border-radius:20px;border:1px solid var(--border2);background:var(--card2);color:var(--text2);font-size:12px;cursor:pointer">
              ✏️ {{ auth.user()?.fullName ? 'Edit name' : 'Set your name' }}
            </button>
          </div>
        } @else {
          <div style="display:flex;gap:8px;align-items:center">
            <input class="fi" [(ngModel)]="editName" placeholder="Your name"
              (keyup.enter)="saveName()" (keyup.escape)="editingName.set(false)" style="flex:1;margin-bottom:0">
            <button class="btn btn-amber btn-sm" [disabled]="savingName()" (click)="saveName()">{{ savingName() ? '...' : 'Save' }}</button>
      ...
      <div class="ph">
        <div class="ph-row">
          <div>
            ...
            <div class="ph-sub">{{ filtered().length }} shops {{ cityFilter() ? 'in ' + cityFilter() : 'available' }}</div>
          </div>
      ...
      @if (loading()) {
        <div class="g3">
          @for (i of [1,2,3,4,5,6]; track i) {
            <div style="background:var(--card);border-radius:18px;height:260px;border:1px solid var(--border);animation:pulse 1.5s infinite"></div>
          }
        </div>
          ...
          @for (shop of filtered(); track shop.id) {
            <div class="shop-card" [class.closed]="!shop.open" (click)="book(shop)">
              ...
              <div class="sc-body">
                ...
                <!-- Employee quick-view -->
                @if (shop.totalEmployees && shop.totalEmployees > 0) {
                  <div class="emp-quick">
                    <span>👤 {{ shop.totalEmployees }} staff</span>
                    @if (shop.freeEmployeesNow !== undefined) {
                      <span [class]="shop.freeEmployeesNow! > 0 ? 'emp-dot-free' : 'emp-dot-busy'" style="width:8px;height:8px;border-radius:50%;display:inline-block"></span>
                      <span [style.color]="shop.freeEmployeesNow! > 0 ? 'var(--emerald)' : 'var(--text3)'">
                        {{ shop.freeEmployeesNow }} free now
                      </span>
                    }
                  </div>
                }
                ...
                <div class="sc-foot">
                  <div>
                    ...
                    <div style="font-size:11px;color:var(--text3);margin-top:2px">From ₹{{ minPrice(shop) }}</div>
                  </div>
                  <button class="btn btn-amber btn-sm" (click)="$event.stopPropagation(); book(shop)">Book →</button>
                </div>
              </div>
            </div>
          }
...
export class CustomerBrowseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  startEditName() { this.editName = this.auth.user()?.fullName || ''; this.editingName.set(true); }
  ...
  applyFilters() {
    ...
    if (this.showNearby() && this.userLat) {
      list = list
        .map(s => ({ ...s, distance: s.latitude && s.longitude ? this.haversine(this.userLat, this.userLng, +s.latitude, +s.longitude) : 999 }))
  ...
  areasForCity(): string[] { const city = this.cityFilter(); const m = this.meta(); if (!city || !m) return []; return m.areasByCity[city] ?? []; }

  toggleNearby() {
    if (!this.showNearby()) {
      navigator.geolocation?.getCurrentPosition(pos => {
        this.userLat = pos.coords.latitude; this.userLng = pos.coords.longitude;
        this.showNearby.set(true); this.applyFilters();
        setTimeout(() => this.initMap(), 100);
      }, () => this.showNearby.set(false));
    } else { this.showNearby.set(false); this.map?.remove(); this.map = null; }
  }
  ...
  private initMap() {
    ...
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap', className: 'dark-tiles' }).addTo(this.map);
    L.circleMarker([this.userLat, this.userLng], { radius: 8, color: '#3b9eff', fillColor: '#3b9eff', fillOpacity: 1 }).bindPopup('📍 You are here').addTo(this.map);
  ...
  book(shop: ShopResponse) { if (!shop.open) return; this.bookShop.emit(shop.id); }
  minPrice(shop: ShopResponse): number { const prices = (shop.services || []).filter(s => s.enabled).map(s => s.price); return prices.length ? Math.min(...prices) : 0; }
  haversine(lat1: number, lng1: number, lat2: number, lng2: number): number {
    const R = 6371; const d = (a: number, b: number) => (b - a) * Math.PI / 180;
