...
@Component({
      ...
      <div class="auth-box" (click)="$event.stopPropagation()">
        ...
        <div class="auth-box-body">
          ...
          @if (step() === 'phone') {
            ...
            <button class="btn btn-ghost btn-block" style="margin-top:8px" (click)="step.set('login'); activeTab.set('barber')">
              Barber / Admin Login →
            </button>
          }
          ...
          @if (step() === 'otp') {
            <div style="text-align:center;margin-bottom:20px;padding:14px;background:linear-gradient(135deg,#1a2e1a,#0f1f0f);border:1px solid rgba(37,211,102,0.2);border-radius:12px">
              ...
              <div style="font-size:13px;color:var(--text2)">OTP sent to <b>+91 {{ phone }}</b> via WhatsApp</div>
            </div>
            <div class="fg">
              <label class="fl">6-Digit OTP</label>
              <input class="fi" [(ngModel)]="otp" type="tel" maxlength="6" placeholder="_ _ _ _ _ _"
                style="text-align:center;font-size:24px;font-family:'Unbounded',sans-serif;letter-spacing:8px"
            ...
            <div style="text-align:center;margin-top:12px">
              @if (resendCooldown() > 0) {
                <span style="font-size:12px;color:var(--text3)">Resend in {{ resendCooldown() }}s</span>
              } @else {
                <button class="btn btn-ghost btn-sm" (click)="resendOtp()">Resend OTP</button>
              }
              <button class="btn btn-ghost btn-sm" style="margin-left:8px" (click)="step.set('phone')">← Change number</button>
            </div>
          }

          <!-- Customer: Set name (new user) -->
          @if (step() === 'name') {
            <div class="fg">
              <label class="fl">Your Name</label>
              <input class="fi" [(ngModel)]="fullName" placeholder="e.g. Rahul Sharma" (keyup.enter)="saveName()">
            </div>
            <button class="btn btn-amber btn-block btn-lg" [disabled]="loading() || !fullName.trim()" (click)="saveName()">
          ...
          @if (step() === 'login') {
            <div class="fg">
              ...
              <input class="fi" [(ngModel)]="email" type="email" placeholder="you@example.com" (keyup.enter)="loginEmail()">
            </div>
            <div class="fg">
            ...
            <div style="display:flex;justify-content:space-between;margin-top:12px">
              <button class="btn btn-ghost btn-sm" (click)="step.set('forgot')">Forgot password?</button>
              @if (activeTab() === 'barber') {
                <button class="btn btn-ghost btn-sm" (click)="step.set('register')">Register shop →</button>
              }
            </div>
            <button class="btn btn-ghost btn-block" style="margin-top:8px" (click)="step.set('phone'); activeTab.set('customer')">
              ← Back to phone login
            </button>
          }
  ...
  `,
  styles: [`
    .auth-overlay { position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:9999;display:flex;align-items:center;justify-content:center;overflow-y:auto; }
    .auth-box { background:var(--surface);border:1px solid var(--border2);border-radius:24px;width:calc(100% - 32px);max-width:420px;margin:auto;box-shadow:var(--shadow-lg);animation:fadeUp 0.3s cubic-bezier(0.22,1,0.36,1); }
    .auth-box-head { padding:28px 28px 0;background:linear-gradient(180deg,var(--card) 0%,transparent 100%); }
    .auth-box-logo { font-family:'Unbounded',sans-serif;font-size:18px;font-weight:900;margin-bottom:16px; }
...
export class AuthModalComponent implements OnInit {
  ...
  phone = ''; otp = ''; email = ''; password = ''; fullName = '';
  ...
  subs: Record<Step, string> = {
    ...
    register: 'Set up your barbershop on BookurBarber',
  ...
  switchTab(tab: Tab): void {
    this.activeTab.set(tab); this.errMsg.set('');
  ...
  verifyOtp(): void {
    ...
    this.auth.verifyOtp(this.phone, this.otp).subscribe({
      next: (res) => {
        ...
        if (res.data?.isNewUser) { this.step.set('name'); }
        else { this.toast.ok('Welcome back! 👋'); this.router.navigate(['/customer']).then(() => this.close.emit()); }
      },
  ...
  saveName(): void {
    ...
    this.auth.updateProfile(this.fullName.trim()).subscribe({
      next: () => { this.loading.set(false); this.toast.ok('Welcome to BookurBarber! 🎉'); this.close.emit(); setTimeout(() => this.router.navigate(['/customer']), 50); },
      error: () => { this.loading.set(false); this.auth.updateUser({ ...this.auth.user()!, fullName: this.fullName.trim() }); this.close.emit(); setTimeout(() => this.router.navigate(['/customer']), 50); }
    });
  }
  ...
  loginEmail(): void {
    ...
    this.auth.login(this.email, this.password).subscribe({
      next: (res) => {
        this.loading.set(false); this.toast.ok('Welcome back!');
  ...
  registerBarber(): void {
    ...
    this.auth.registerBarber(this.reg).subscribe({
      next: () => { this.loading.set(false); this.toast.ok('Shop registered! Awaiting admin approval.'); this.step.set('login'); },
  ...
  forgotPassword(): void {
    ...
    this.auth.forgotPassword(this.email).subscribe({
      next: () => { this.loading.set(false); this.toast.ok('Reset link sent!'); this.step.set('login'); },
  ...
  private startResendCooldown(): void {
    ...
    const t = setInterval(() => { this.resendCooldown.update(v => v - 1); if (this.resendCooldown() <= 0) clearInterval(t); }, 1000);
  }
}
